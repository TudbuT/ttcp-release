...
public class BlockUtils {
    ...
    public static BlockPos getRealPos(Vec3d vec3d) {
        return new BlockPos((int)(vec3d.x < 0 ? Math.floor(vec3d.x) : vec3d.x), (int)(vec3d.y < 0 ? Math.floor(vec3d.y) : vec3d.y), (int)(vec3d.z < 0 ? Math.floor(vec3d.z) : vec3d.z));
    }
    ...
    public static boolean clickOnBlock(BlockPos pos, EnumHand hand, boolean rotate) {
        ...
        mc.player.connection.sendPacket(new CPacketPlayerTryUseItemOnBlock(pos, EnumFacing.UP, hand, f, f1, f2));
    ...
    public static Vec2f getLegitRotationsVector(Vec3d vec) {
        ...
        double yaw = Math.toDegrees(Math.atan2(diffZ, diffX)) - 90f;
        double pitch = (-Math.toDegrees(Math.atan2(diffY, diffXZ)));
        return new Vec2f((float) MathHelper.wrapDegrees(yaw), (float) MathHelper.wrapDegrees(pitch));
    }
