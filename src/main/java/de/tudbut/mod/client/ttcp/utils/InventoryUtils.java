...
public class InventoryUtils {
    ...
    public static void setCurrentSlot(int id) {
        if(TTCp.player.inventory.currentItem != id) {
            TTCp.player.inventory.currentItem = id;
            TTCp.player.connection.sendPacket(new CPacketHeldItemChange(id));
        }
    }
    
    // Get selected hotbar slot
    public static int getCurrentSlot() {
        return TTCp.player.inventory.currentItem;
    }
    ...
    public static void drop(int windowId, int slot) {
        clickSlot(windowId, slot, ClickType.THROW, 1);
    }
    
    // Virtually clicks a slot
    public static void clickSlot(int slot, ClickType type, int key) {
        TTCp.mc.playerController.windowClick(TTCp.mc.player.inventoryContainer.windowId, slot, key, type, TTCp.mc.player);
    }
    
    // Virtually clicks a slot
    public static void clickSlot(int windowId, int slot, ClickType type, int key) {
        TTCp.mc.playerController.windowClick(windowId, slot, key, type, TTCp.mc.player);
    }
